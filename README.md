...
### [Elastalert](#index)
  ...
  * /elastalert
  > python3 -m elastalert.elastalert --verbose --config <config.yaml> // (config.yaml 파일 내 rules_folder: <설정된폴더> 를 따라 룰 파일을 땡겨와 실행한다. 
  >여러 룰 파일을 사용하고 싶으면 이 명령어를 사용한다. 본 매뉴얼을 처음 따라온다면 아래 명령어를 통해 따라한다.)
